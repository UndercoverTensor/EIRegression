...
def execute(save_dir, n_buckets=3, i=None, bucketing_method="quantile", single_rules_breaks=3):
    ...
    domain_data = pd.read_csv(
        os.path.join(dataset_path, "delta_elevators.domain"),
        delim_whitespace=True, header=None)

    data = pd.read_csv(
        os.path.join(dataset_path, "delta_elevators.data"),
        delim_whitespace=True, header=None)
...
if __name__ == '__main__':
    save_dir = os.path.join(PROJECT_ROOT, "experiments/XGBRegression/results/")
